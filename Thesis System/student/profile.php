...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ((isset($_POST['first_name']) || isset($_POST['program_id'])) && !isset($_POST['current_password'])) {
        ...
        if (isset($_POST['program_id'])) {
            if ($userModel->updateProgram(Auth::id(), $_POST['program_id'])) {
                if (empty($message)) $message = 'Program updated successfully!';
                else $message .= ' Program updated successfully!';
            } else {
                if (empty($message)) $message = 'Error updating program. Please try again.';
                else $message .= ' Error updating program. Please try again.';
            }
        }
...
// Get current user details and programs
$userModel = new User($pdo);
$current_user = $userModel->getUserWithDetails(Auth::id());
...
<div class="row" style="display: flex; flex-wrap: wrap; margin: 0 -15px;">
        ...
        <div class="card fade-in">
            ...
            <div class="card-body" style="padding: 25px;">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group" style="margin-bottom: 20px;">
                        <label for="student_id" style="display: block; margin-bottom: 8px; font-weight: 500;">Student ID</label>
                        <input type="text" id="student_id" name="student_id" class="form-control" style="width: 100%; padding: 12px; border: 2px solid #e1e5e9; border-radius: 10px; font-family: 'Poppins', sans-serif;" value="<?php echo htmlspecialchars($current_user['student_id']); ?>" readonly>
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom: 20px;">
                        ...
                        <select id="program_id" name="program_id" class="form-control" style="width: 100%; padding: 12px; border: 2px solid #e1e5e9; border-radius: 10px; font-family: 'Poppins', sans-serif;">
                            <?php foreach ($programs as $program): ?>
                                <option value="<?php echo $program['id']; ?>" <?php echo ($current_user['program_id'] == $program['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($program['name']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
    ...
    <div class="col-md-4" style="flex: 0 0 33.333%; padding: 0 15px;">
        ...
        <div class="card fade-in">
            ...
            <div class="card-body" style="padding: 25px; text-align: center;">
                <form method="POST" enctype="multipart/form-data">
                    <div style="width: 200px; height: 100px; border: 2px dashed #ccc; margin: 0 auto 20px; display: flex; align-items: center; justify-content: center;">
